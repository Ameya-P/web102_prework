...
function addGamesToPage(games) {
    ...
    for (const game of games) {
        ...
        gameCard.innerHTML = `<img src=${game.img} height='128'>
        <h3>${game.name}</h3>
        <p>${game.description}</p>
        <div id="money">
        <p>goal <br> ${game.goal}</p>
        <p>pledged <br> ${game.pledged}</p>
        <p>backers <br> ${game.backers}</p>
        </div>`;
...
Currently ${numUnfunded} ${numUnfunded == 1 ? "game remains" : "games remain"} unfunded. ${numUnfunded == 0 ? "Thank you for funding every game!" : "We need your help to fund these amazing games!"}`;
...
top.textContent = firstGame.name;

firstGameContainer.appendChild(top);

// do the same for the runner up item
const sec = document.createElement('p');
sec.textContent = secondGame.name;
